...
class Assembler:
    def assemble(self, lines):
        ...
        # gives decimal numbers bach
        compiled = [
            self._compile(instr, labels) for instr in instructions
        ]
        # Changes the decimals numbers to hexadecimals
        program = self._to_text(compiled)
        return program
# [/class]

    # [labels] -> loc does not increment with label. Does not count as address.
    # We get for loop the loc two that we know that on the next line we got this label
    ...
    def _compile(self, instruction, labels):
        ...
        # Final code is stored as arg1, arg 0, op
        if fmt == "--":
        ...
        # for labels and store
            ...
            # bei ldc R2 1 -> 1, 2, codeindex
            # bei bne R2 @loop -> 2, 2, codeindex
            tmp = self._combine(
    ...
    # [/compile]
    # bei val int return - bei label -> position of line after last label
    # [value]
